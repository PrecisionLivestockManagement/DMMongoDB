...
#' @param paddock the name of the paddock that the ALMS rating will be applied to
...
update_pdkrating <- function(property, paddock, ALMSrating=NULL, condition=NULL, LTCC=NULL, username=NULL, password=NULL){
  ...
  for (i in 1:length(paddock)){
    pdk <- sprintf('{"stationname":"%s", "paddname":"%s"}', property, paddock[i])

    if(!(is.na(pdk))){
      banger <- paddocks$find(query = filterpaddock, fields = '{"stationname":true, "paddname":true, "ALMSrating":true, "condition":true, "LTCC":true}')
