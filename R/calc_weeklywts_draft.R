...
#' @param values this is the minimum number of weight values that need to be recorded to calculate an average weekly weight. The default is 4 weight values.
...
#' @param remove.duplicates if TRUE, this allows duplicate date time values to be removed from the weekly weight calculation. The default is FALSE.
#' @param username if you don't have a username set up using the dmaccess function you can pass a username, if no value added then the function looks for a value from dmaccess via keyring
#' @param password if you include a username you will also need to add a password contact Lauren O'Connor if you don't have access
#' @return A list that includes a list of the RFID numbers that have been returned and a dataframe for each RFID number that provides date and weekly weight statistics.
...
calc_weeklywts <- function(RFID, start = NULL, end = NULL, values = NULL, s.d = NULL, remove.duplicates = FALSE, username = NULL, password = NULL){
  ...
  if(is.null(remove.duplicates)){remove.duplicates <- "FALSE"}
