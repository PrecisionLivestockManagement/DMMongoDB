...
updatepaddock <- function(RFID, property, paddock, addtoherd, date=NULL, username=NULL, password=NULL){
  ...
  filterinfs <- sprintf('{"stationname":"%s", "properties.Paddock":{"$in":["%s"]}, "properties.type":"%s"}', property, paddock, "Walk-over-Weighing Unit")
  ...
  for (i in 1:length(RFID)){
    ...
    RFIDS <- sprintf('{"RFID":"%s"}', RFID[i])
          ...
          if (banger$properties$Paddock != paddock){
          ...
          if (ALMS == "TRUE"){
            ...
            WOW <- inf[which(inf$properties$Paddock == paddock[i]),]
              ...
              IDI <- sprintf('{"$set":{"properties.ALMSdateON":{"$date":"%s"}", properties.ALMSdateOFF":{"$date":"%s"}, "properties.ALMS":"%s", "properties.ALMSID":"%s", "properties.ALMSasset_id":"%s"}}',
                             paste0(substr(date[i],1,10),"T","00:00:00","+1000"), paste0("1970-01-01","T","00:00:00","+1000"), "TRUE", WOW$`_id`, WOW$properties$asset_id)}
